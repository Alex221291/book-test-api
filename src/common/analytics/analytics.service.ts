...
export class AnalyticsService {
	...
	async getRevenue(hash: string, dateStart: string, dateFinish: string) {
		const timeFrom = date(dateStart);
		const timeTo = date(dateFinish);
		...
		return (await builder.getRawOne())?.value || 0;
	}

	async getAverage(hash: string, dateStart: string, dateFinish: string) {
		const timeFrom = date(dateStart);
		const timeTo = date(dateFinish);
		const builder = this.paymentsService.findWithQueryBuilder();
		builder
	...
	async revenueByEmployee(hash: string, dateStart: string, dateFinish: string) {
		const timeFrom = date(dateStart);
		const timeTo = date(dateFinish);
		const builder = this.paymentsService.findWithQueryBuilder();
		builder
	...
	async revenueByServices(hash: string, dateStart: string, dateFinish: string) {
		const timeFrom = date(dateStart);
		const timeTo = date(dateFinish);
		const builder = this.paymentsService.findWithQueryBuilder();
		builder
	...
	async customerVisitsByService(customerId: number, serviceIds: number[]) {
		...
		builder
			...
			.andWhere(`e.status IN (:statuses)`, {
				statuses: [BookingsStatuses.COMPLETED, BookingsStatuses.PAID],
			})
	...
	async getServicesRate(
		...
		builder
			...
			.andWhere('e.status NOT IN(:status)', {
				status: [BookingsStatuses.CANCELLED],
			})
			.andWhere(
				`(schedule.sinceDate BETWEEN '${timeFrom.toSQL()}' AND '${timeTo.toSQL()}' OR schedule.untilDate BETWEEN '${timeFrom.toSQL()}' AND '${timeTo.toSQL()}')`,
			)
	...
	async getCancelledBookings(
		...
		builder
			...
			.innerJoinAndSelect('e.schedules', 'schedule')
			.innerJoinAndSelect('office.company', 'company')
			.andWhere(`e.status = '${BookingsStatuses.CANCELLED}'`)
			.andWhere(`company.hash = '${companyHash}'`)
			.andWhere(
				`(schedule.sinceDate BETWEEN '${timeFrom.toSQL()}' AND '${timeTo.toSQL()}' OR schedule.untilDate BETWEEN '${timeFrom.toSQL()}' AND '${timeTo.toSQL()}')`,
			);
